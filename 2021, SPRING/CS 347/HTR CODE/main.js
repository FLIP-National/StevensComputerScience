...
function getRandInput(){
    ...
    document.getElementById("Speed").innerHTML = speed;     // do not change.

    // RPM: Rotations Per Minute on the train wheels
    rpm = Math.floor(Math.random() * 1000);                 // rpm = <number>
    document.getElementById("RPM").innerHTML = rpm;         // do not change.

    // Longitude: X location of the train.
    longitude = Math.floor(Math.random() * 180);            // longitude = <number>
    document.getElementById("Long").innerHTML = longitude;  // do not change.

    // Latitude: Y location of the train.
    latitude = Math.floor(Math.random() * 90);              // latitude = <number>
    document.getElementById("Lat").innerHTML = latitude;    // do not change.

    // Etemp: Engine Temperature of the train.
    etemp = Math.floor(Math.random() * 190);                // etemp = <number>
    document.getElementById("EngTemp").innerHTML = etemp;   // do not change.

    // Otemp: Outside Temperature of the train.
    otemp = Math.floor(Math.random() * 100);                // otemp = <number>
    document.getElementById("OutTemp").innerHTML = otemp;   // do not change.

    // ActualSpeed: Calculates the speed from RPM.
    actualSpeed = Math.floor((2.89 * rpm / 1609) * 60);     // actualSpeed = <number>
    document.getElementById("RPMs").innerHTML = actualSpeed;// do not change.
