...
let rec prune n t =
  ...
  | (0,_) -> Empty
  | (_,Empty) -> Empty
  | (level, Node(h, left, right)) ->
          Node(h, prune (level-1) left, prune (level-1) right)
...
let rec per_helper' l =
  match l with 
  | [] -> truep
  | h::h2::t -> 
      if h=h2 then per_helper' t
      else false

let perfect t = 
  if height t == 1
  then true
  else per_helper' (map length_list (paths_to_leaves t)) 
