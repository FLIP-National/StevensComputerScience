...
let rec chk_expr : expr -> texpr tea_result = fun e ->
  ...
  | Untuple(ids,def,target) ->
    ...
    chk_expr target
    else error "untuple: ids have incorrect dimensions"
  | Record(fs) ->
    let (id, exs) = List.split fs 
    in sequence (List.map chk_expr exs) >>= fun ts ->
    return (RecordType(List.combine id ts))
  | Proj(e, id) ->
    chk_expr e >>=
    list_of_recordVal >>= fun ts ->
    (match List.assoc_opt id ts with 
      | None -> error "not found"
      | Some ev -> extend_list_tenv id ts)
  | Debug(_e) ->
