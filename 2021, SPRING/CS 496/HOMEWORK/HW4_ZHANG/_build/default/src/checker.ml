...
declaration")
  ...
  | SetRef(e1,e2) ->
    type_of_expr e1 >>= 
    arg_of_refType "setref: " >>= fun v1 -> 
    type_of_expr e2 >>= fun v2 ->
    return UnitType
  ...
  | Cons(h, t) ->
    ...
    type_of_expr t >>= fun tl ->
    (match (hd, tl) with
    | t, ListType(x) -> return (ListType x)
    |_ ->  failwith "cons: type of head and tail do not match")
  | Null(e) ->
    type_of_expr e >>= arg_of_listType "null: " >>= fun lst ->
    return BoolType 
  | Hd(e) ->
    type_of_expr e >>= arg_of_listType "hd: " >>= fun lst ->
    return lst
  | Tl(e) ->
    type_of_expr e >>= arg_of_listType "tl: " >>= fun lst ->
    return (ListType lst) 
  ...
  | NullT(t) ->
    type_of_expr t >>= arg_of_treeType "null: " >>= fun lst ->
    return BoolType  
  | GetData(t) ->
    (match t with 
    | Node(i,_,_) ->
        type_of_expr i >>= fun e ->
        return e 
    | _ -> error "Empty tree")
  | GetLST(t) ->
    (match t with 
