...
let rec removeHelper l len counter = 
  match l with 
  | [] -> []
  | h::t -> 
    if len = counter 
    then []
    else h :: (removeHelper t len (counter+1))
...
and
  ...
  | AddQ(e1,e2) ->
    eval_expr e1 >>= fun a ->
    eval_expr e2 >>= 
    list_of_queueVal >>= fun b ->
    return (QueueVal (a :: b))
  | Element(e) ->
    eval_expr e >>= 
    list_of_queueVal >>= fun a ->
    if a = []
    then error "Element: Queue is empty." 
            (*on doc said: "Remove: Queue is Empty"
              wasn't sure if that was a typo? *)
    else return  (List.hd (List.rev a))
  | Remove(e) ->
    eval_expr e >>= 
    list_of_queueVal >>= fun a ->
    if a = []
    then error "Remove: Queue is empty."
    else return (QueueVal (removeHelper a (List.length a) 1))
  | IsEmpty(e) ->
    eval_expr e >>= 
    list_of_queueVal >>= fun a ->
    if a = []
